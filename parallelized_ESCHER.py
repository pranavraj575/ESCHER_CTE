...
class ESCHERSolver(policy.Policy):
    def __init__(self,
                 game,
                 create_worker_group=False,
                 num_experience_workers=6,
        ...
        Args:
          ...
          create_worker_group: Set to True when running distributed version, False if worker or not distributed
          num_experience_workers: The number of ray parallel workers
        ...
        self._num_experience_workers = num_experience_workers
        ...
        for player in range(self._num_players):
            if create_worker_group:
                ...
                self._worker_regret_networks.append(
                    RegretNetwork(self._embedding_size, self._regret_network_layers,
                                  self._num_actions))
            if create_worker_group:
        ...
        if create_worker_group:
            self.workers = []
            for _ in range(num_experience_workers):
                self.workers.append(ray.remote(num_cpus=1)(ESCHERSolver).remote(game,
                                                                                create_worker_group=False,
                                                                                num_experience_workers=num_experience_workers,
                                                                                ...
                                                                                batch_size_average_policy=batch_size_average_policy,
                                                                                ...
                                                                                save_regret_networks=save_regret_networks,
                                                                                save_average_policy_memories=save_average_policy_memories,
                                                                                expl=expl,
                                                                                val_bootstrap=val_bootstrap,
                                                                                all_actions=all_actions,
                                                                                val_expl=val_expl,
                                                                                importance_sampling_threshold=importance_sampling_threshold,
                                                                                importance_sampling=importance_sampling,
                                                                                oshi_zumo=oshi_zumo,
                                                                                markov_soccer=markov_soccer,
    ...
    def get_all_num_calls(self):
        return [ray.get(worker.get_num_calls.remote()) for worker in self.workers]

    def sync_val_weights(self):
        val_weights = self.get_val_weights()
        for worker in self.workers:
            ray.get(worker.set_val_weights.remote(val_weights))
    ...
    def _update_iterations(self):
        ...
        for worker in self.workers:
            ray.get(worker.set_iteration.remote(iteration))
    ...
    def set_weights(self, weights):
        for player in range(self._num_players):
            self._worker_regret_networks[player].set_weights(weights[player])

    def sync_worker_weights(self):
        current_regret_weights = self.get_weights()
        for worker in self.workers:
            ray.get(worker.set_weights.remote(current_regret_weights))
    ...
    def sync_worker_policy_weights(self):
        current_policy_weights = self.get_policy_weights()
        for worker in self.workers:
            ray.get(worker.set_policy_weights.remote(current_policy_weights))
    ...
    def clear_value_memory_test(self):
    ...
    def clear_val_memories_test(self):
        ...
        for w in self.workers:
            ray.get(w.clear_value_memory_test.remote())

    def clear_val_memories(self):
        self._value_memory.clear()
        for w in self.workers:
            ray.get(w.clear_value_memory.remote())
    ...
    def init_regret_net(self):
        ...
        for p in range(self._num_players):
            example_info_state = ray.get(self.workers[0].get_example_info_state.remote(p))
            example_legal_actions_mask = ray.get(
                self.workers[0].get_example_legal_actions_mask.remote(p))
            for w in self.workers:
                ray.get(w.traverse_game_tree_n_times.remote(1, p, track_mean_squares=False))
            self._init_main_regret_network(example_info_state, example_legal_actions_mask, p)

    def init_val_net(self):
        example_hist_state = ray.get(self.workers[0].get_example_hist_state.remote())
        example_legal_actions_mask = ray.get(
            self.workers[0].get_example_legal_actions_mask.remote(0))
    ...
    def print_mse(self):
        # track MSE
        squared_errors = []
        squared_errors_child = []
        for w in self.workers:
            squared_errors.extend(ray.get(w.get_squared_errors.remote()))
    ...
    def solve(self, save_path_convs=None):
        ...
        with tf.device(self._infer_device):
            with contextlib.ExitStack() as stack:
                ...
                convs = []
                nodes = []
                ray.get(self.workers[0].traverse_game_tree_n_times.remote(1, 0, track_mean_squares=False))
                ...
                for i in range(self._num_iterations + 1):
                    ...
                    results = []
                    for w in self.workers:
                        result = w.traverse_game_tree_n_times.remote(self._num_val_fn_traversals, 0,
                                                                     train_value=True,
                                                                     track_mean_squares=False,
                                                                     on_policy_prob=self._val_op_prob,
                                                                     expl=self._val_expl)
                        result2 = w.traverse_game_tree_n_times.remote(20, 0,
                                                                      train_value=True,
                                                                      track_mean_squares=False,
                                                                      on_policy_prob=self._val_op_prob,
                                                                      expl=self._val_expl,
                    ...
                    print(val_train_time - reinit_time, 'val train time')
                    ...
                    for p in range(self._num_players):
                        ...
                        num_nodes = []
                        for w in self.workers:
                            result = w.traverse_game_tree_n_times.remote(self._num_traversals, p,
                                                                         train_regret=True,
                                                                         track_mean_squares=track_mse,
                                                                         expl=self._expl)
                            results.append(result)
                            num_nodes.append(w.get_num_nodes.remote())
                        num_nodes = sum(ray.get(num_nodes))
    ...
    def _get_matched_regrets(self, info_state, legal_actions_mask, player):
        """TF-Graph to calculate regret matching."""
        regrets = self._worker_regret_networks[player](
            (tf.expand_dims(info_state, axis=0), legal_actions_mask),
            training=False)[0]
    ...
    def _get_regret_dataset(self, player):
        ...
        worker_datas = []
        for w in self.workers:
            worker_data = w.get_regret_memories.remote(player)
            worker_datas.append(worker_data)
        for worker_data in worker_datas:
            worker_data = ray.get(worker_data)
            random.shuffle(worker_data)
            data.extend(worker_data)
    ...
    def _get_value_dataset(self):
        ...
        worker_datas = []
        for w in self.workers:
            worker_data = w.get_value_memory.remote()
            worker_datas.append(worker_data)
        for worker_data in worker_datas:
            worker_data = ray.get(worker_data)
            random.shuffle(worker_data)
    ...
    def _get_value_dataset_test(self):
        ...
        worker_datas = []
        for w in self.workers:
            worker_data = w.get_value_memory_test.remote()
            worker_datas.append(worker_data)
        for worker_data in worker_datas:
            worker_data = ray.get(worker_data)
            random.shuffle(worker_data)
    ...
    def _get_average_policy_dataset(self):
        ...
        if self._memories_tfrecordpath:
            ...
            worker_datas = []
            for w in self.workers:
                worker_data = w.get_average_policy_memories.remote()
                worker_datas.append(worker_data)
            for worker_data in worker_datas:
                worker_data = ray.get(worker_data)
                random.shuffle(worker_data)
                data.extend(worker_data)
            # print(len(data), 'len average_policy data')
            data = tf.data.Dataset.from_tensor_slices(data)
...
if __name__ == "__main__":
    ...
    regret, pol_loss, convs, nodes = ray.get(deep_cfr_solver.solve.remote(save_path_convs=save_path))
